...
### Production Model
- **`models/final_mixed.pth`** — PyTorch CRNN, final epoch, 97.3% corrected val
- **`models/crnn.onnx`** — ONNX export of the same weights, 100% prediction parity with PyTorch on all validation samples. Ship this. 6.4 MB. Loads cleanly via `onnxruntime` and `cv2.dnn` (≥4.8.0)

### Deployment
- `scripts/predict.py` — PyTorch inference (development/debug), loads `models/final_mixed.pth`
- `scripts/predict_onnx.py` — ONNX inference (production), loads `models/crnn.onnx`. Depends only on `onnxruntime` + `Pillow` + `numpy`. Use as the reference implementation when porting to other runtimes (`onnxruntime-web`, `cv2.dnn`, mobile, etc.)
- `scripts/export_onnx.py` — re-exports `models/final_mixed.pth` to `models/crnn.onnx` and verifies parity on both val sets
...
models/                   crnn.onnx, final_mixed.pth, yolov8n baseline
